...
let itemBox3 = document.querySelectorAll('.itemBox3');
...
for (let i = 0; i<list.length; i++){
	list[i].addEventListener('click', function(){
		...
		for( let k = 0; k<itemBox3.length; k++){
			itemBox3[k].classList.remove('active');
			itemBox3[k].classList.add('hide');
			
			if(itemBox3[k].getAttribute('data-item') == dataFilter ||
			dataFilter == "all"){
				itemBox3[k].classList.remove('hide');
				itemBox3[k].classList.add('active');
		}
	}
})
} 
